...
Nos interesa sebr cuál es la \textit{situación conyugal} de esas jefas de hogar a los efectos de determinar si son jefas de hogar porque no tienen pareja o si teniendo pareja declararon ser jefas de hogar, lo cual sería evidencia de \textbf{empoderamiento femenino}.
...
-  Crear una variable que se llame \textit{porcentaje} que indique el porcentaje de mujeres jefas de hogar para cada categoría de la variable situación conyugal (`f2`).
...
```{r}
tareas <- personas %>% 
      filter(!is.na(f12), !is.na(g5), f12 != 9)
```
